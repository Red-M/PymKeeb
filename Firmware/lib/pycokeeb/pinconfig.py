...
class PinConfig(BaseKeeb):
    ...
    def pin_setup(self):
        ...
        if self.leds_enabled==True:
            self.led_spi = [
                self._led_spi(board.GP19,board.GP18,4),
                self._led_spi(board.GP10,board.GP11,110)
            ]
        if self.screen_enabled==True:
            self.screen_spi = self._screen_spi(board.GP10,board.GP11,board.GP13,board.GP14,board.GP15)
